...
rational_approx t = t - ((0.010328 * t + 0.802853) * t + 2.515517) / (((0.001308 * t + 0.189269) * t + 1.432788) * t + 1.0);
...
levelGenerator s = scanl getNextXOrigin (safeZone 0.0) makeObjects
    where
        -- makeObjects [Double -> [GameObject]] [Double -> [GameObject]]
...
generateRandomWalls s xOrigin = zipWith3 makeWall platformXOrigins platformYOrigins platformLenghts
        where
            ...
            platformYOrigins = map (\x -> ((screenHeight * 2) / yLevels) * x - screenHeight) (take numberOfWalls uniformRvs)
...
generateRandomSpikes s xOrigin = zipWith makeSpike platformXOrigins platformYOrigins
        where
            ...
            platformYOrigins = map (\x -> ((screenHeight * 2) / yLevels) * x - screenHeight + 1.5) (take numberOfspikes uniformRvs)
...
-- TODO: rewrite these two functions on polymorphic functions
